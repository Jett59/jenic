...
jenic::syntax::Function::Function(jenic::Token return_type, jenic::Token name,
...
jenic::AbstractSyntaxNode* jenic::Parser::parseFunction(int *index) {
    ...
    while (tokens[i].value != ")") {
        ...
        if (type.type != jenic::TOKEN_IDENTIFIER
                || name.type != TOKEN_IDENTIFIER) {
            std::cerr << "Error: unexpected token " << type.value << " "
                    << name.value << "at index " << i << std::endl;
            std::exit(-1);
        }
